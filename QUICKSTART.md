...
# Опционально: для работы с реальными дронами
pip install dronekit dronekit-sitl

# Опционально: для работы с симулятором AirSim
pip install airsim
```

### Шаг 2: Проверка системы (1 минута)
...
### Шаг 3: Первый полет (2 минуты)

#### Вариант A: Интерактивная демонстрация

```bash
python demo.py
```

Выберите из предложенных демонстраций:
- 1. Базовые команды (взлет, зависание, посадка)
- 2. Телеметрия (получение данных)
- 3. Выполнение миссии
- 4. Инструменты

#### Вариант B: Запуск агента
...
Используйте команды:
```
1. Статус       - Получить статус агента
2. Телеметрия   - Получить телеметрию
3. Команда      - Отправить команду (взлет/посадка/rtl/зависни)
4. Миссия       - Запустить миссию
5. Субагент     - Вопрос ИИ-помощнику
6. Выход        - Завершить работу
```

#### Вариант C: API сервер
